...
| `fix-igpu.sh` | Intel iGPU boot args by MUX mode (read from the `MsiDCVarData` UEFI var): **Discrete** → blacklist `i915,xe` (an output-less i915 blanks the LUKS prompt); **MSHybrid** → `xe.force_probe=a788` (kernel 7.0+ dropped i915 for this Raptor Lake iGPU; without it the panel + Huawei went dark). |
...
- **iGPU driver depends on MUX mode** (`fix-igpu.sh` picks): in MSHybrid mode kernel 7.0+ needs `xe.force_probe=a788` or the Intel-driven outputs go dark; in Discrete mode (current) `i915` and `xe` are **blacklisted** (`rd.driver.blacklist` + `modprobe.blacklist`) because the iGPU has no outputs and an output-less DRM device breaks the boot splash (next gotcha). Re-run `sudo bash fix-igpu.sh` after every MUX switch.
