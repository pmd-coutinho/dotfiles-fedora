...
- **restarting quickshell**: the process is named `quickshell`, not `qs` — `pkill -x qs` misses it and stale instances keep drawing their old windows (duplicate bars/menus). Use `pkill -x quickshell; qs -d`.
