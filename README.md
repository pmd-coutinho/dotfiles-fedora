...
- **Compositor**: niri, **rendering on the NVIDIA dGPU** (`debug { render-drm-device }` in `config.kdl`, live since 2026-08-26): the 4K144 Gigabyte is wired to the RTX 4080, so rendering there means only the two small Intel-side outputs pay for cross-GPU copies instead of every 3840x2160@144 frame. Not hot-reloadable; comment it out again if the 4K ever moves back to an Intel port. 3 monitors: Huawei top-left, laptop below it, Gigabyte (4K@144 via DSC) right.
