...
- **Lock screen (2026-09 redesign) verified only in `LockPreview`** — layout,
  PAM prompt and the wrong-password shake. Still to confirm on a real
  `Super+Alt+L`: the fade-out actually releases the lock (the `releaseGuard`
  timer is the belt-and-braces), the accent ring follows keyboard focus between
  outputs, and monitors stay off while locked (quickshell-mirror#503).
