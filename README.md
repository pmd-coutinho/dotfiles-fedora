...
| `fix-igpu.sh` | `xe.force_probe=a788` — kernel 7.0+ dropped i915 for this Raptor Lake iGPU; without it the iGPU has no driver (in MSHybrid mode that meant the laptop panel + Huawei went dark). |
...
- **iGPU on kernel 7.0+**: needs `xe.force_probe=a788` (`fix-igpu.sh`) or the Intel GPU has no driver. Only matters for outputs in MSHybrid mode; in Discrete mode (current) the iGPU has no outputs anyway.
...
- **LUKS prompt invisible under `rhgb quiet` in Discrete mode**: Fedora's plymouth uses simpledrm only when there is no LUKS, otherwise it waits for a real KMS driver; RPM Fusion omits nvidia from the initramfs and the iGPU has no outputs, so the graphical splash has nowhere to draw. `setup-round4.sh` pins `UseSimpledrm=1` in `/etc/plymouth/plymouthd.conf` and rebuilds the initramfs. Blind-typing the passphrase still works if it ever regresses, or drop `rhgb quiet` at the GRUB menu for a text prompt.
